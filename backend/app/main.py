...
@app.get("/api/agents")
def agents(): return {"agents":AGENT_NAMES}
...
def brief(c):
    return {"product_name":c.product_name,"description":c.description,"target_audience":c.target_audience,
            "objective":c.objective,"budget":c.budget,"duration_days":c.duration_days,
            "platforms":[x for x in c.platforms.split(",") if x],"brand_tone":c.brand_tone,
            "brand_guidelines":c.brand_guidelines}

def serialize_content(x):
    return {"id":x.id,"campaign_id":x.campaign_id,"platform":x.platform,"content_type":x.content_type,
            "topic":x.topic,"body":x.body,"approval_status":x.approval_status,
            "publishing_status":x.publishing_status,"review_status":x.review_status,
            "review_notes":x.review_notes,"scheduled_date":x.scheduled_date}

@app.post("/api/campaigns/{cid}/run")
def run_campaign(cid:int,db:Session=Depends(get_db)):
    c=db.get(Campaign,cid)
    if not c: raise HTTPException(404,"Campaign not found")
    # Re-running a campaign should replace its generated package rather than duplicate it.
    # Delete dependent approval rows first so PostgreSQL foreign-key constraints are respected.
    old_content_ids=[x.id for x in db.query(ContentItem.id).filter(ContentItem.campaign_id==cid).all()]
    if old_content_ids:
    ...
    c.workflow_status="awaiting_human_approval"; c.status="active"
    c.projected_analytics=json.dumps(result.get("analytics",{}))
    outputs={
    ...
    for name in AGENT_NAMES:
        db.add(AgentRun(campaign_id=cid,agent_name=name,status="completed",output=json.dumps(outputs.get(name,""), default=str)))
    for item in result["reviewed_content"]:
        ci=ContentItem(campaign_id=cid,platform=item["platform"],content_type=item["content_type"],
                       topic=item["topic"],body=item["body"],review_status=item["review_status"],
                       review_notes=item["review_notes"],scheduled_date=item["scheduled_date"])
        db.add(ci); db.flush(); db.add(ApprovalStep(content_id=ci.id,status="pending"))
    db.commit()
    return {"campaign_id":cid,"workflow_status":c.workflow_status,"agents":AGENT_NAMES,
            "strategy":result["strategy"],"research":result["research"],
            "analytics":result["analytics"],"content_count":len(result["reviewed_content"])}
...
def edit_content(content_id:int,data:ContentUpdate,db:Session=Depends(get_db)):
    ...
    if not x: raise HTTPException(404,"Content not found")
    x.body=data.body; x.approval_status="pending"; x.publishing_status="not_published"
    from app.agents.content_review_agent import ContentReviewBrandComplianceAgent
    result=ContentReviewBrandComplianceAgent().run({"brief":brief(db.get(Campaign,x.campaign_id)),"content":[{"platform":x.platform,"content_type":x.content_type,"topic":x.topic,"body":x.body,"scheduled_date":x.scheduled_date}]})
    checked=result["reviewed_content"][0]
    x.review_status=checked["review_status"]; x.review_notes=checked["review_notes"]
    db.commit(); return {"status":"updated","id":x.id,"review_status":x.review_status,"review_notes":x.review_notes}
...
def approve(content_id:int,data:ApprovalRequest,db:Session=Depends(get_db)):
    ...
    x.approval_status="approved"; x.review_notes=data.comment or x.review_notes
    step=db.query(ApprovalStep).filter_by(content_id=content_id).first()
    if step: step.status="approved"; step.comment=data.comment
    campaign=db.get(Campaign,x.campaign_id)
    db.commit()
    if campaign:
        pending=db.query(ContentItem).filter(ContentItem.campaign_id==campaign.id,ContentItem.approval_status!="approved").count()
        if pending==0:
            campaign.workflow_status="calendar_ready"; db.commit()
...
def reject(content_id:int,data:ApprovalRequest,db:Session=Depends(get_db)):
    x=db.get(ContentItem,content_id)
    if not x: raise HTTPException(404,"Content not found")
    x.approval_status="rejected"; x.review_notes=data.comment or "Rejected for revision"
    step=db.query(ApprovalStep).filter_by(content_id=content_id).first()
    if step: step.status="rejected"; step.comment=data.comment
...
def regenerate(content_id:int,db:Session=Depends(get_db)):
    ...
    if not c: raise HTTPException(404,"Campaign not found")
    b=brief(c)
    draft=ContentGenerationAgent()._generate(x.platform,b,x.topic,{})
    x.body=draft
    x.approval_status="pending"; x.publishing_status="not_published"
    from app.agents.content_review_agent import ContentReviewBrandComplianceAgent
    checked=ContentReviewBrandComplianceAgent().run({"brief":b,"content":[{"platform":x.platform,"content_type":x.content_type,"topic":x.topic,"body":x.body,"scheduled_date":x.scheduled_date}]})["reviewed_content"][0]
    x.review_status=checked["review_status"]; x.review_notes=checked["review_notes"]
    db.commit(); return {"status":"regenerated","content":serialize_content(x)}

@app.post("/api/content/{content_id}/publish")
def publish(content_id:int,db:Session=Depends(get_db)):
    x=db.get(ContentItem,content_id)
    if not x: raise HTTPException(404,"Content not found")
    if x.approval_status!="approved": raise HTTPException(409,"Publishing is blocked until human approval.")
    x.publishing_status="published"; db.commit(); return {"status":"published"}
...
def analytics(cid:int,data:dict,db:Session=Depends(get_db)):
    ...
    if result.get("observed"): c.observed_analytics=json.dumps(result)
    else: c.projected_analytics=json.dumps(result)
    db.commit()
    return result

@app.post("/api/campaigns/{cid}/analytics/upload")
def analytics_upload(cid:int,file:UploadFile=File(...),db:Session=Depends(get_db)):
    if not db.get(Campaign,cid): raise HTTPException(404,"Campaign not found")
    try:
    ...
    if any(v < 0 for v in totals.values()): raise HTTPException(400,"CSV KPI values cannot be negative")
    ...
    c=db.get(Campaign,cid); c.observed_analytics=json.dumps(result); db.commit()
    return result
...
def report_data(cid,db):
    ...
    try: strat=json.loads(outputs.get("Campaign Strategy Agent","{}"))
    except Exception: strat=orch.strategy.run({"brief":b})["strategy"]
    try: research_data=json.loads(outputs.get("Audience & Competitor Research Agent","[]"))
    except Exception: research_data=orch.research.run(b)["research"]
    analytics_data=json.loads(c.observed_analytics or "{}") or json.loads(c.projected_analytics or "{}") or CampaignAnalyticsAgent().run({"impressions":10000,"engagements":700,"clicks":450,
...
def export_csv(cid:int,db:Session=Depends(get_db)):
    c,items,_,_,_=report_data(cid,db)
...
def report(cid:int,db:Session=Depends(get_db)):
    c,items,strat,research_data,analytics_data=report_data(cid,db)
    return {"campaign":brief(c),"strategy":strat,"research":research_data,"analytics":analytics_data,
            "content_calendar":[serialize_content(x) for x in items],
            "approval":{"approved":sum(x.approval_status=="approved" for x in items),

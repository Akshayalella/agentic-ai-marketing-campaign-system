...
allowed_origins = list(
    dict.fromkeys(
        ...
        + [
            "http://localhost:5173",
            "http://127.0.0.1:5173",
            "https://agentic-ai-marketing-campaign-syste.vercel.app",
        ]
    )
)
...
def _prepare_campaign(db, c):
    old_content_ids = [
        ...
        .filter(ContentItem.campaign_id == c.id)
        .all()
    ]
...
def _worker(cid, stop_event):
    ...
    try:
        ...
        print(
            f"Campaign {cid} workflow error: {exc}"
        )
...
def update_campaign(
    ...
    with running_lock:
        if cid in running_events:
            raise HTTPException(
                409,
                "Stop the workflow before editing this campaign.",
            )
    ...
    c.target_audience = data.target_audience
    c.objective = data.objective
    c.budget = data.budget
    c.duration_days = data.duration_days
    c.platforms = ",".join(data.platforms)
    c.brand_tone = data.brand_tone
    c.brand_guidelines = data.brand_guidelines
...
def run_campaign(
    ...
    with running_lock:
        if cid in running_events:
            raise HTTPException(
                409,
                "Campaign workflow is already running.",
            )
...
def stop_campaign(
    ...
    if not event or c.workflow_status != "running":
        raise HTTPException(
            409,
            "Campaign workflow is not currently running.",
        )
...
def delete_campaign(
    ...
    with running_lock:
        if cid in running_events:
            raise HTTPException(
                409,
                "Stop the workflow before deleting this campaign.",
            )
...
# ============================================================
# RESEARCH / STRATEGY / CONTENT / CALENDAR
# ============================================================
...
def strategy(
    ...
    result = orch.strategy.run(
        {
            "brief": brief(c)
        }
    )["strategy"]

    result["total_budget"] = float(
        c.budget or 0
    )

    result["budget_allocation_total"] = round(
        sum(
            result.get(
                "budget_allocation",
                {}
...
# ============================================================
# CONTENT EDIT / APPROVAL / REGENERATION / PUBLISH
# ============================================================
...
def approve(
    ...
    if x.publishing_status == "published":
        raise HTTPException(
            409,
            "Published content is already final.",
        )
    ...
    x.review_notes = (
        data.comment or x.review_notes
    )
...
def regenerate(
    ...
    if x.publishing_status == "published":
        raise HTTPException(
            409,
            "Published content cannot be regenerated.",
        )

    b = brief(c)

    x.body = ContentGenerationAgent()._generate(
        x.platform,
        b,
        x.topic,
        {},
...
def publish(
    ...
    if x.review_status not in (
        ...
        raise HTTPException(
            409,
            "Publishing is blocked until automated review passes.",
        )
...
def analytics(
    ...
    if result.get("observed"):
        c.observed_analytics = json.dumps(
            result
        )
    else:
        c.projected_analytics = json.dumps(
...
def analytics_upload(
    ...
    if (
        ...
        or totals["clicks"]
        > totals["impressions"]
        or totals["conversions"]
        > totals["clicks"]
    ):
        raise HTTPException(
            400,
            "CSV KPI relationships are invalid: "
            "engagements/clicks cannot exceed impressions "
            "and conversions cannot exceed clicks",
        )

    try:
        parsed_targets = json.loads(
            targets or "{}"
        )
        ...
        raise HTTPException(
            400,
            "Invalid analytics targets.",
        )

    totals["observed"] = True
    totals["targets"] = parsed_targets

    result = CampaignAnalyticsAgent().run(
        totals
    )
...
def report_data(cid, db):
    ...
    if c.observed_analytics:
        try:
            analytics_data = json.loads(
                c.observed_analytics
            )
    ...
    if (
        not analytics_data
        and c.projected_analytics
    ):
        try:
            analytics_data = json.loads(
                c.projected_analytics
            )
